create function shp.dl_set_tracking() 
returns void
language plpgsql
as $$
declare
        ...
        ***/
        ...
        begin
            update "Счета" b set ("КодТК", "ДокТК", "ДокТКДата", "Отгружен") = (6, dl_tr.tracking_code, dl_tr.shipment_dt, true)
                where b."№ счета" = dl_tr."№ счета";
            loc_status := 1;
            loc_result1 := dl_tr."№ счета"::varchar;
        ...
        /***/
        RAISE NOTICE 'dl_tr=%, loc_result=%, loc_result1=%', dl_tr,loc_result, loc_result1;
    end loop;
end;$$